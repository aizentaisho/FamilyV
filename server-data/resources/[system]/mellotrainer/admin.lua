...
RegisterNUICallback("time", function(data, cb)
	...
	if (action == "input") then
		...
		if (request == nil) or (request > 24) or (request < 0) then
			drawNotification("~r~Vous devez spécifier un nombre entre 0 et 24.")
			return
		else
			h=request
			m=0
	...
	drawNotification("~g~Temps changé à "..tostring(h)..":"..string.format("%02d", tostring(m)))
end)
...
RegisterNUICallback("weatheroptions", function(data, cb)
	...
	if(action == "wind")then
		...
		drawNotification("Vent "..text)
	elseif(action == "freeze")then
		featureWeatherFreeze = newstate
		drawNotification("Météo verrouillée "..text.."~w~, sélectionnez la météo à appliquer")
...
RegisterNUICallback("weather", function(data, cb)
	...
	drawNotification("Météo changée à ~g~" .. weather .. ".")
	cb("ok")
end)
...
AddEventHandler("mellotrainer:adminstatus", function(status)
	if(status)then
		...
		drawNotification("~r~Vous n'êtes pas un admin !")
	end
end)
