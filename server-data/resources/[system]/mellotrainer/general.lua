...
function teleportToWaypoint()
	...
	if(not IsWaypointActive())then
		drawNotification("Aucun marqueur détecté.")
		return
	end
	...
	drawNotification("Téléporté au marqueur.")
end
...
AddEventHandler("mellotrainer:adminStatusReceived", function(status)
	Citizen.Trace("Votre statut d'admin : "..tostring(status))
	adminStatus = status
end)
		...
		if(IsPedInAnyVehicle(playerPed))then
			if(playerPed == GetPedInVehicleSeat(playerVeh, -1))then
				...
				if(not(inVeh))then
					...
					Citizen.Trace("Application des options de véhicule")
